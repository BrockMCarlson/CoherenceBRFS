...
plot(t1,v1) xlabel('Time [s]'); ylabel('Voltage [mV]'); 
...
plot(pow) xlabel('Indices []'); ylabel('Power [dB]'); 
...
plot(faxis, pow); xlim([0 50]) xlabel('Frequency [Hz]') ylabel('Power [dB]') 
...
[S,f] = mtspectrumc(v1, params); S = 10*log10(S/max(S)); plot(f,S); xlim([0 50]) xlabel('Frequency [Hz]') ylabel('Power [dB]') 
%% %%%% Begin Example 2. %%%% 
%%%% We'll now focus on variables 'v2' and 't2' %%%% 
%% Example 2: Plot the data. 
plot(t2,v2) xlabel('Time [s]'); 
...
S = abs(S); imagesc(T,F,10*log10(S/max(S(:)))); colorbar; axis xy ylim([0 20]) xlabel('Time [s]') ylabel('Freq [Hz]') 
...
colorbar; axis xy ylim([0 20]) xlabel('Time [s]') ylabel('Freq [Hz]') 
...
plot(t3,v3a(1,:)) subplot(2,1,2) %Plot the data from v3b, trial 2. 
plot(t3,v3b(2,:)) xlabel('Time [s]') 
...
plot(faxis, mean(powA(:,1:ttrials/2+1),1)) xlim([0 50]) xlabel('Frequency [Hz]') ylabel('Power [dB]') subplot(1,2,2) plot(faxis, mean(powB(:,1:ttrials/2+1),1)) xlim([0 50]) xlabel('Frequency [Hz]') ylabel('Power [dB]') 
...
%coherence*. 
plot(faxis, coh(1:length(coh)/2+1)) xlim([0 50]); ylim([0 1]) xlabel('Frequency [Hz]') ylabel('Coherence') 
...
params.tapers = [2 4]; % See Chronux.org params.trialave=1; 
