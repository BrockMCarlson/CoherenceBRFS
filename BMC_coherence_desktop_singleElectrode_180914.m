...
function [stimWin] = triggerStim(timePoints,LFP)
...
stimWin = struct('stimLFP_full',{nan},'stimLFP_bl',{nan},'stimLFP_target',{nan});
stimWin.stimLFP_full = nan(size(timePoints,1),pre+post+1,size(LFP,2));
stimWin.stimLFP_bl = nan(size(timePoints,1),pre_bl+1,size(LFP,2));
stimWin.stimLFP_target = nan(size(timePoints,1),post-postTargetStart+1,size(LFP,2));

    for tr = 1:length(timePoints) % trigger to stim-on times for all trials
        stimtm = round(timePoints(tr,1)/30) ;% divide by 30 to convert to 1kHz. Note, LFP and MUA already in 1kHZ
        refwin = stimtm-pre:stimtm+post;
        refwin_bl =  stimtm-pre_bl:stimtm;
        refwin_target = stimtm+postTargetStart:stimtm+postTargetStop;
        stimWin.stimLFP_full(tr,:,:)        = LFP(refwin,:);
        stimWin.stimLFP_bl(tr,:,:)     = LFP(refwin_bl,:);
        stimWin.stimLFP_target(tr,:,:) = LFP(refwin_target,:);
